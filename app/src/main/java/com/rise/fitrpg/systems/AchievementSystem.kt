...
import com.rise.fitrpg.data.models.AchievementCondition
...
import com.rise.fitrpg.data.models.MAX_STAT_VALUE
import com.rise.fitrpg.data.models.PersonalRecord
import com.rise.fitrpg.data.models.StatType
import com.rise.fitrpg.data.models.TimeOperator
...
object AchievementSystem {
    ...
    private const val MIN_PR_IMPROVEMENT_PERCENT = 20.0
    ...
    // All data that the evaluator needs after a workout is saved
    // One thing to be careful about: previousLastWorkoutDateMs must be captured BEFORE!e!e!

    // classesSwitchedTo was removed as a separate field user was already carrieing it
    // Having two copies meant the caller had to keep them in sync, and one could drift silently
    // Basicly it had a bug and at 1.something it got fixxed
    data class AchievementContext(
        ...
        val previousPersonalRecords: List<PersonalRecord>,
        val totalQuestsCompleted: Int,
        val balanceQuestsCompleted: Int,
        val latestSessionTimestampMs: Long,
    ...
    // Evaluates every achievement, returns newly unlocked ones and updated progress for all
    ...
    // Returns true if the new PR beats the previous one by at least MIN_PR_IMPROVEMENT_PERCENT
    // Handles all three tracking types - old version only handled REPS_WEIGHT VER 2.0
    // so bodyweight userss (pull-ups, plank, etc.) could never unlock any PRCount achievement
    // No previous PR of the same type = first PR, always counts
    // PR with no populated metric field = malformed, does not count
    ...
    // Local hour (0 to 23) from a Unix ms timestamp
    // Uses device timezone so Early Bird and Night Owl trigger on local time, not UTC
    // Phase 2: replace with Android Calendar API
    ...
    // 0=Mon, 1=Tue ... 5=Sat, 6=Sun in local timezone
    // Timezone offset applied before calculation without it, users near midnight in non-UTC
     ...
     */
    // checks if user logged in saturday 23:00 or later AND the following Sunday before 07:00 in the same weekend
