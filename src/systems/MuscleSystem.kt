...
// MuscleSystem.kt
// Layer: Systems
// Communicates with: Models only (MuscleScore, MuscleRank,
//                    PlayerProfile, PersonalRecord, Exercise,
//                    CardioExercise, CardioSession, MuscleGroup)
...
//                    coefficient applied so gender/age still matters for cardio muscles
//
// Only the exercise PRIMARY muscle group drives the score. Secondary muscles ignored.
// Muscle scores are global - never reset on class switch.
// HIP_FLEXORS excluded from human model - no MuscleScore produced for it.
//
// ============================================================

object MuscleSystem {

    // TUNABLE - adjust so solid cardio performance lands around B rank (score ~1.0-1.5)
    // Example: 15km run, male 20s (coeff 1.0) - 15 / (12.0 x 1.0) = 1.25 - B rank
    // Same run, female 20s (coeff 0.75) - 15 / (12.0 x 0.75) = 1.67 - A rank
    ...
    /**
     * Recalculates all muscle scores from scratch after every session.
     * Returns only what changed plus any rank-ups for UI notifications.
     */
    fun recalculateAll(
        ...
        for (muscleGroup in MuscleGroup.entries) {
            ...
            // Fix: bestReps cast to Double explicitly for clear division intent
            val repsScore = prs
    ...
    /** Returns the MuscleScore for a single muscle - for the UI detail screen. */
    ...
    /** Returns all ranked muscles sorted strongest first - for the UI overview. */
    ...
    /** Returns all unranked muscles - for UI suggestions on what to train next. */
    ...
    /**
     * Calculates cardio score for cardio-driven muscles.
     * Formula: bestDistanceKm / (CARDIO_SCALE_FACTOR x coefficient)
     * Fix: coefficient applied so gender/age normalization is consistent with other methods.
     * Returns null if no distance-based cardio sessions exist.
     */
    ...
    /** Converts a raw score to a MuscleRank by finding the highest threshold met. */
