...
// StatSystem.kt
// Layer: Systems
// Communicates with: Models only (Stats, StatContribution,
...
object StatSystem {
    ...
    // Stat growth is capped at MAX_STAT_VALUE — imported from models layer
    ...
    /**
     * Returned after every stat update.
    ...
    // --------------------------------------------------------
    // PUBLIC FUNCTIONS — STRENGTH / FLEXIBILITY
    // --------------------------------------------------------

    /**
     * Calculates and applies stat gains from a single strength or flexibility set.
     * Called once per WorkoutSet after XP has been calculated by XpSystem.
     *
     * @param currentStats  The player's current stat values
     * @param exercise      The exercise being logged
     * @param finalXp       The final XP earned for this set (after all multipliers)
     * @return StatGainResult with updated stats and a breakdown of gains
     */
    ...
    /**
     * Calculates and applies stat gains from a complete cardio session.
     * Called once per CardioSession after XP has been calculated by XpSystem.
     *
     * @param currentStats      The player's current stat values
     * @param cardioExercise    The cardio exercise definition
     * @param finalXp           The final XP earned for this session (after all multipliers)
     * @return StatGainResult with updated stats and a breakdown of gains
     */
    ...
    /**
     * Adds Focus stat from streak progression.
     * Called by the caller (ViewModel in Phase 2) after StreakSystem.evaluateWeek()
     * returns GoalMet — streak days × FOCUS_XP_PER_STREAK_DAY.
     *
     * Focus is not tied to any exercise StatContribution — it rewards consistency.
     *
     * @param currentStats      The player's current stat values
     * @param streakDays        Current streak day count after the weekly evaluation
     * @return StatGainResult with updated stats (only focusGained will be non-zero)
     */
    ...
    /**
     * Adds Focus stat from completing a quest.
     * Called by the caller after QuestSystem returns a newly completed quest.
     *
     * @param currentStats  The player's current stat values
     * @return StatGainResult with updated stats (only focusGained will be non-zero)
     */
    ...
    /**
     * Calculates the gain for a single stat field.
     * Formula: (finalXp × weight).toInt(), capped so stat never exceeds MAX_STAT.
     */
    ...
    /**
     * Builds a StatGainResult by diffing before and after stats.
     * Used by Focus-only functions where only one field changes.
     */
