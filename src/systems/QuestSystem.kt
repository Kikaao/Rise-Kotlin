...
// QuestSystem.kt
// Layer: Systems
// Communicates with: Models only (Quest, QuestModels, User,
//                    WorkoutSession, CardioSession, StreakTier)
...
//
// Phase 2: AI replaces generateClassQuests() and generateBalanceQuests()
...
object QuestSystem {
     ...
     * Called when a new week starts (detected by QuestSystem caller
     * comparing nowMs to user.weekStartDate + WEEK_IN_MS).
     *
     * Quest counts by StreakTier minimum:
     *   PEASANT  → 1 class + 1 balance (min 1 balance always)
     *   KNIGHT   → 3 class + 2 balance
     *   ELITE    → 5 class + 3 balance
     *
     * @param user                  Current user
     * @param allWorkoutSessions    Full session history — used for neglect detection
     * @param allCardioSessions     Full cardio history — used for neglect detection
    ...
    /**
     * Updates progress on all active quests after a workout session is logged.
     * Called automatically after every session save.
     * Returns updated quest list, newly completed quests, and rewards earned.
     *
     * @param activeQuests          Current active quests for this user
     * @param workoutSession        The session just logged (null if cardio only)
     * @param cardioSession         The cardio session just logged (null if strength only)
     * @param user                  Current user (for streak check)
     * @param nowMs                 Current timestamp in Unix ms
     */
    ...
    /**
     * Returns true if a quest has passed its expiry timestamp.
     * Expired quests are never evaluated — they stay as-is until the week resets.
     */
    ...
    /**
     * Returns the number of class quests for a given StreakTier.
     * Based on the tier's minimum workout days.
     */
     ...
     * Ceiling of (minimum days / 2), minimum 1 always.
     *
     * PEASANT: ceil(1/2) = 1
     * KNIGHT:  ceil(3/2) = 2
     * ELITE:   ceil(5/2) = 3
    ...
    fun getBalanceQuestCount(streakTier: StreakTier): Int {
        ...
        return maxOf(1, (minimum + 1) / 2) // ceiling division, min 1
    }
    ...
    /**
     * Generates class quests optimized for the user's current class.
     * Distributes across quest types and rarities to give variety.
     * Phase 2: this function is replaced by an AI call — same Quest output.
     */
    ...
    /**
     * Generates balance quests targeting neglected stats/classes.
     * Neglect is detected by checking when the relevant exercises were
     * last logged — if gap >= NEGLECT_THRESHOLD_DAYS, a balance quest fires.
     *
     * If not enough neglected areas are found, falls back to
     * the least-trained class.
     *
     * Phase 2: replaced by AI — same Quest output.
     */
    ...
    /**
     * Builds a single Quest from its parameters.
     * All tunable values (targets, rewards) are sourced from constants above.
     */
    ...
    // --------------------------------------------------------
    // PRIVATE — QUEST PROGRESS UPDATE
    // --------------------------------------------------------

    /**
     * Updates progress on a single quest based on what was just logged.
     * Each quest type checks a different field from the session.
     *
     * Fix: COMPLETE_WORKOUTS and LOG_MUSCLE_GROUP_SETS now verify that the session's
     * fitnessClass matches the quest's targetClass. Previously, any workout counted toward
     * any class quest — a balance quest for CHAMPION could be satisfied by logging a
     * MONK session, which defeats the entire purpose of balance quests. The quest
     * description ("…as ${targetClass}") already promised this enforcement — now the
     * code matches the description.
     *
     * Class quests (isBalanceQuest = false) have targetClass = user.currentClass, so the
     * enforcement is a no-op for them as long as the user hasn't switched class mid-week.
     * If they switch class mid-week, class quests for the OLD class stop progressing —
     * which is the correct behavior (you're not that class anymore).
     */
    ...
    // --------------------------------------------------------
    // PRIVATE — NEGLECT DETECTION
    // --------------------------------------------------------

    /**
     * Finds classes whose primary training area has been neglected.
     * Neglect = no relevant sessions logged in NEGLECT_THRESHOLD_DAYS.
