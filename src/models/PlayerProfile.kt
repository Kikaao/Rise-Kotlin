...
data class PlayerProfile(
    val userId: Int,
    val gender: Gender,
    val ageYears: Int,                  // exact age — converted to AgeBracket internally
    val heightCm: Double,
    val weightKg: Double                // bodyweight — used as denominator in rank formula
) {
...
object ProfileCoefficients {
    ...
    // Age coefficients — TUNABLE
    // Rule (per v2.1 decisions log #35 and doc §4.9): lower coefficient = same
    // performance scores higher. The formula is score = metric / (bodyweightKg × coeff),
    // so smaller coefficient → smaller denominator → higher score.
    //
    // Peak strength is 20s–30s → baseline coefficient 1.00.
    // Teens are still developing → higher coefficient (mild penalty — not bonus).
    // 40s and 50s+ → lower coefficient (bonus — same lift at 50 is more impressive).
    //
    // Fix: values were inverted — old values were FORTIES=1.05, FIFTIES_PLUS=1.10
    // (both HIGHER than baseline), which produced a penalty for older athletes, directly
    // contradicting the stated intent and decision #35. Old TEENS=0.85 was also inverted:
    // it was documented as a penalty but the number gave teens a bonus. Now all five
    // values follow the single rule "lower = rewarded more."
    ...
    // Combined coefficient — multiply both together
    // Used by MuscleSystem as a single divisor adjustment
